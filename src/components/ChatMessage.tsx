...
export const ChatMessage = ({ message, isStreaming = false }: ChatMessageProps) => (
  ...
  >
    <div className="flex items-start w-full max-w-3xl gap-4 px-4 mx-auto">
      {message.role === 'assistant' ? (
        <div className="flex items-center justify-center flex-shrink-0 w-8 h-8 text-sm font-medium text-white rounded-lg bg-gradient-to-r from-blue-500 to-blue-600">
          AI
        </div>
      ) : (
